			   ...
			   uint gpuaddr, int sizedwords)
{
	...
	KGSL_CMD_INFO(dev_priv->device, "ib: gpuaddr:0x%08x, wc:%d, hptr:%pKK\n",
	...
	while (dwords_left > 0) {
		...
		default:
			KGSL_CMD_ERR(dev_priv->device, "unexpected type: "
				"type:%d, word:0x%08x @ 0x%pKK, gpu:0x%08x\n",
		...
		if (dwords_left < 0) {
			KGSL_CMD_ERR(dev_priv->device,
				"bad count: c:%d, #:%d/%d, "
				"v:0x%08x @ 0x%pKK[gb:0x%08x], level:%d\n",
