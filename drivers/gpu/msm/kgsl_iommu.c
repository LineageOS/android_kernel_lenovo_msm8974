	...
	struct device *dev, unsigned long addr, int flags, void *token)
{
	...
	if (!iommu_dev) {
		KGSL_CORE_ERR("Invalid IOMMU device %pKK\n", dev);
		ret = -ENOSYS;
		goto done;
	}
	iommu = mmu->priv;
	...
	for (i = 0; i < iommu->unit_count; i++) {
		...
		for (j = 0; j < iommu_unit->dev_count; j++) {
			...
			if (iommu_unit->dev[j].attached) {
				...
				KGSL_MEM_INFO(mmu->device, "iommu %pKK detached "
					"from user dev of MMU: %pKK\n",
					iommu_pt->domain, mmu);
			}
		}
	}
	...
	for (i = 0; i < iommu->unit_count; i++) {
		...
		for (j = 0; j < iommu_unit->dev_count; j++) {
			...
			if (!iommu_unit->dev[j].attached) {
				...
				"iommu pt %pKK attached to dev %pKK, ctx_id %d\n",
	...
	struct kgsl_device_iommu_data *data, unsigned int unit_id)
{
	...
	for (j = 0; j < KGSL_IOMMU_MAX_DEVS_PER_UNIT; j++) {
		...
		KGSL_DRV_INFO(mmu->device,
				"Obtained dev handle %pKK for iommu context %s\n",
				iommu_unit->dev[iommu_unit->dev_count].dev,
		...
		unsigned int *tlb_flags)
{
	...
	if (ret) {
		KGSL_CORE_ERR("iommu_unmap_range(%pKK, %x, %d) failed "
			...
			unsigned int *tlb_flags)
{
	...
	ret = iommu_map_range(iommu_pt->domain, iommu_virt_addr, memdesc->sg,
				size, protflags);
	if (ret) {
		KGSL_CORE_ERR("iommu_map_range(%pKK, %x, %pKK, %d, %x) err: %d\n",
	...
	if (kgsl_memdesc_has_guard_page(memdesc)) {
		...
		if (ret) {
			KGSL_CORE_ERR("iommu_map(%pKK, %x, guard, %x) err: %d\n",
