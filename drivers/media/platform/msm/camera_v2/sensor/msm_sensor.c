...
int msm_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (!power_info || !sensor_i2c_client) {
		pr_err("%s:%d failed: power_info %pKK sensor_i2c_client %pKK\n",
...
int msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (!power_info || !sensor_i2c_client || !slave_info ||
		!sensor_name) {
		pr_err("%s:%d failed: %pKK %pKK %pKK %pKK\n",
...
int32_t msm_sensor_init_default_params(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (!s_ctrl) {
		pr_err("%s:%d failed: invalid params s_ctrl %pKK\n", __func__,
			__LINE__, s_ctrl);
		return -EINVAL;
	}

	if (!s_ctrl->sensor_i2c_client) {
		pr_err("%s:%d failed: invalid params sensor_i2c_client %pKK\n",
	...
	s_ctrl->sensor_i2c_client->cci_client = kzalloc(sizeof(
		struct msm_camera_cci_client), GFP_KERNEL);
	if (!s_ctrl->sensor_i2c_client->cci_client) {
		pr_err("%s:%d failed: no memory cci_client %pKK\n", __func__,
	...
	if (!clk_info) {
		pr_err("%s:%d failed no memory clk_info %pKK\n", __func__,
			__LINE__, clk_info);
		rc = -ENOMEM;
