...
int32_t msm_sensor_driver_probe(void *setting)
{
	...
	if (!setting) {
		pr_err("failed: slave_info %pKK", setting);
		return -EINVAL;
	}
	...
	if (!slave_info) {
		pr_err("failed: no memory slave_info %pKK", slave_info);
		return -ENOMEM;
	}
	...
	if (!s_ctrl) {
		pr_err("failed: s_ctrl %pKK for camera_id %d", s_ctrl,
			slave_info->camera_id);
		rc = -EINVAL;
		goto FREE_POWER_SETTING;
	}

	CDBG("s_ctrl[%d] %pKK", slave_info->camera_id, s_ctrl);
	...
	if (!power_setting) {
		pr_err("failed: no memory power_setting %pKK", power_setting);
		rc = -ENOMEM;
		goto FREE_SLAVE_INFO;
	}
	...
	if (!power_down_setting) {
		pr_err("failed: no memory power_setting %pKK",
						power_down_setting);
		rc = -ENOMEM;
		goto FREE_POWER_SETTING;
	}
	...
	if (!camera_info) {
		pr_err("failed: no memory slave_info %pKK", camera_info);
		goto FREE_POWER_DOWN_SETTING;
	...
	if (!s_ctrl->sensor_i2c_client) {
		pr_err("failed: sensor_i2c_client %pKK",
	...
	if (!cci_client) {
		pr_err("failed: cci_client %pKK", cci_client);
		goto FREE_CAMERA_INFO;
	}
	...
	struct device_node *of_node)
{
	...
	if (!sensordata || !of_node) {
		pr_err("failed: invalid params sensordata %pKK of_node %pKK",
			sensordata, of_node);
		return -EINVAL;
	}
...
static int32_t msm_sensor_driver_parse(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	s_ctrl->sensor_i2c_client = kzalloc(sizeof(*s_ctrl->sensor_i2c_client),
		GFP_KERNEL);
	if (!s_ctrl->sensor_i2c_client) {
		pr_err("failed: no memory sensor_i2c_client %pKK",
			s_ctrl->sensor_i2c_client);
		return -ENOMEM;
	}
	...
	if (!s_ctrl->msm_sensor_mutex) {
		pr_err("failed: no memory msm_sensor_mutex %pKK",
	...
	pr_err("g_sctrl[%d] %pKK", s_ctrl->id, g_sctrl[s_ctrl->id]);
...
static int32_t msm_sensor_driver_platform_probe(struct platform_device *pdev)
{
	...
	if (!s_ctrl) {
		pr_err("failed: no memory s_ctrl %pKK", s_ctrl);
		return -ENOMEM;
	}
