...
static long msm_sensor_driver_cmd(struct msm_sensor_init_t *s_init, void *arg)
{
	...
	if (!s_init || !cfg) {
		pr_err("failed: s_init %pKK cfg %pKK", s_init, cfg);
		return -EINVAL;
	}
	...
	unsigned int cmd, void *arg)
{
	...
	/* Validate input parameters */
	if (!s_init) {
		pr_err("failed: s_init %pKK", s_init);
		return -EINVAL;
	}
...
static int __init msm_sensor_init_module(void)
{
	...
	if (!s_init) {
		pr_err("failed: no memory s_init %pKK", NULL);
		return -ENOMEM;
	}

	pr_err("MSM_SENSOR_INIT_MODULE %pKK", NULL);
