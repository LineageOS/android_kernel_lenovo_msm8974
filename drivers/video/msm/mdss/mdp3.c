...
static int mdp3_res_init(void)
{
	...
	if (IS_ERR_OR_NULL(mdp3_res->ion_client)) {
		pr_err("msm_ion_client_create() return error (%pKK)\n",
				mdp3_res->ion_client);
...
static int mdp3_parse_bootarg(struct platform_device *pdev)
{
	...
	if (!disp_idx) {
		pr_err("%s:%d:cmdline panel not set disp_idx=[%pKK]\n",
	...
	if (end_idx <= disp_idx) {
		pr_err("%s:%d:cmdline pan incorrect end=[%pKK] disp=[%pKK]\n",
...
void mdp3_unmap_iommu(struct ion_client *client, struct ion_handle *handle)
{
	...
	if (!meta) {
		WARN(1, "%s: buffer was never mapped for %pKK\n", __func__,
				handle);
		mutex_unlock(&mdp3_res->iommu_lock);
		goto out;
	}
...
static void mdp3_iommu_meta_add(struct mdp3_iommu_meta *meta)
{
	...
	while (*p) {
		...
		if (meta->table < entry->table) {
			...
			pr_err("%s: handle %pKK already exists\n", __func__,
				entry->handle);
			BUG();
		}
	}
	...
	unsigned int padding, unsigned long flags)
{
	...
	ret = iommu_map_range(domain, meta->iova_addr + padding,
			table->sgl, size, prot);
	if (ret) {
		pr_err("%s: could not map %lx in domain %pKK\n",
	...
	unsigned long iommu_flags)
{
	...
	if (!iommu_meta) {
		...
		if (iommu_meta->flags != iommu_flags) {
			pr_err("%s: handle %pKK is already mapped with diff flag\n",
				__func__, handle);
			ret = -EINVAL;
			goto out_unlock;
		} else if (iommu_meta->mapped_size != iova_length) {
			pr_err("%s: handle %pKK is already mapped with diff len\n",
				__func__, handle);
			ret = -EINVAL;
			goto out_unlock;
		} else {
		...
		int client)
{
	...
	if (!ret && (img->offset < data->len)) {
		...
		pr_debug("mem=%d ihdl=%pKK buf=0x%x len=0x%x\n", img->memory_id,
...
static int mdp3_alloc(struct msm_fb_data_type *mfd)
{
	...
	pr_info("allocating %u bytes at %pKK (%lx phys) for fb %d\n",
