...
int mdss_fb_alloc_fb_ion_memory(struct msm_fb_data_type *mfd, size_t fb_size)
{
	...
	pr_debug("alloc 0x%zuB vaddr = %pKK (%pKKa iova) for fb%d\n", fb_size,
		...
		struct vm_area_struct *vma)
{
	...
	if (page) {
		for_each_sg(table->sgl, sg, table->nents, i) {
			...
			pr_debug("vma=%pKK, addr=%x len=%ld",
...
static int mdss_fb_alloc_fbmem_iommu(struct msm_fb_data_type *mfd, int dom)
{
	...
	rc = msm_iommu_map_contig_buffer(phys, dom, 0, size, SZ_4K, 0,
					    &mfd->iova);
	if (rc)
		pr_warn("Cannot map fb_mem %pKKa to IOMMU. rc=%d\n", &phys, rc);

	pr_debug("alloc 0x%zxB @ (%pKKa phys) (0x%pKK virt) (%pKKa iova) for fb%d\n",
