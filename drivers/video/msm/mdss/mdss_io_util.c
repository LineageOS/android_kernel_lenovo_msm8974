...
void dss_reg_w(struct dss_io_data *io, u32 offset, u32 value, u32 debug)
{
	...
	if (!io || !io->base) {
		DEV_ERR("%pKKS->%s: invalid input\n",
			__builtin_return_address(0), __func__);
		return;
	}

	if (offset > io->len) {
		DEV_ERR("%pKKS->%s: offset out of range\n",
			__builtin_return_address(0), __func__);
		return;
	}
	...
	struct dss_io_data *io_data, const char *name)
{
	...
	if (!res) {
		DEV_ERR("%pKKS->%s: '%s' msm_dss_get_res_byname failed\n",
	...
	if (!io_data->base) {
		DEV_ERR("%pKKS->%s: '%s' ioremap failed\n",
			__builtin_return_address(0), __func__, name);
		return -EIO;
	}
	...
	int num_vreg, int config)
{
	...
	if (config) {
		for (i = 0; i < num_vreg; i++) {
			...
			if (rc) {
				DEV_ERR("%pKKS->%s: %s get failed. rc=%d\n",
					 __builtin_return_address(0), __func__,
			...
			if (type == DSS_REG_LDO) {
				...
				if (rc < 0) {
					DEV_ERR("%pKKS->%s: %s set vltg fail\n",
...
int msm_dss_enable_vreg(struct dss_vreg *in_vreg, int num_vreg, int enable)
{
	...
	if (enable) {
		for (i = 0; i < num_vreg; i++) {
			...
			if (rc) {
				DEV_ERR("%pKKS->%s: %s regulator error. rc=%d\n",
					__builtin_return_address(0), __func__,
			...
			if (rc < 0) {
				DEV_ERR("%pKKS->%s: %s set opt m fail\n",
					__builtin_return_address(0), __func__,
					in_vreg[i].vreg_name);
				goto vreg_set_opt_mode_fail;
			}
			...
			if (rc < 0) {
				DEV_ERR("%pKKS->%s: %s enable failed\n",
					__builtin_return_address(0), __func__,
					in_vreg[i].vreg_name);
				goto disable_vreg;
			}
...
int msm_dss_clk_set_rate(struct dss_clk *clk_arry, int num_clk)
{
	...
	for (i = 0; i < num_clk; i++) {
		if (clk_arry[i].clk) {
			...
			DEV_ERR("%pKKS->%s: '%s' is not available\n",
				__builtin_return_address(0), __func__,
				clk_arry[i].clk_name);
			rc = -EPERM;
			break;
...
int msm_dss_enable_clk(struct dss_clk *clk_arry, int num_clk, int enable)
{
	...
	if (enable) {
		for (i = 0; i < num_clk; i++) {
			DEV_DBG("%pKKS->%s: enable '%s'\n",
				__builtin_return_address(0), __func__,
				clk_arry[i].clk_name);
			if (clk_arry[i].clk) {
				rc = clk_prepare_enable(clk_arry[i].clk);
				if (rc)
					DEV_ERR("%pKKS->%s: %s en fail. rc=%d\n",
						__builtin_return_address(0),
						__func__,
						clk_arry[i].clk_name, rc);
			} else {
