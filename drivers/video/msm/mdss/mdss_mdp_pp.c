		...
		bool *bl_out_notify)
{
	...
	if (ret == -ENODEV) {
		...
		pr_err("Failed to get ad info: ret = %d, ad = 0x%pKK.\n",
				ret, ad);
		return ret;
	}
	...
	if (!ad->bl_mfd || !ad->bl_mfd->panel_info ||
			!ad->bl_att_lut) {
		pr_err("Invalid ad info: bl_mfd = 0x%pKK, ad->bl_mfd->panel_info = 0x%pKK, bl_att_lut = 0x%pKK\n",
				ad->bl_mfd,
...
int mdss_mdp_hist_intr_req(struct mdss_intr *intr, u32 bits, bool en)
{
	...
	int ret = 0;
	if (!intr) {
		pr_err("NULL addr passed, %pKK", intr);
		return -EINVAL;
	}
...
static int pp_ad_invalidate_input(struct msm_fb_data_type *mfd)
{
	...
	ret = mdss_mdp_get_ad(mfd, &ad);
	if (ret || !ad) {
		pr_err("Fail to get ad: ret = %d, ad = 0x%pKK\n", ret, ad);
		return -EINVAL;
	}
