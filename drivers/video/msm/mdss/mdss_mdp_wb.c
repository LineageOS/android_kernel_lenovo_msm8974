...
struct mdss_mdp_data *mdss_mdp_wb_debug_buffer(struct msm_fb_data_type *mfd)
{
	...
	if (IS_ERR_OR_NULL(ihdl)) {
		...
		if (IS_ERR_OR_NULL(ihdl)) {
			pr_err("unable to alloc fbmem from ion (%pKK)\n", ihdl);
			return NULL;
		}
		...
		pr_debug("ihdl=%pKK virt=%pKK phys=0x%lx iova=0x%x size=%u\n",
	...
	if (!list_empty(&wb->register_queue)) {
		...
		list_for_each_entry(node, &wb->register_queue, registered_entry)
			if ((node->buf_data.p[0].srcp_ihdl == ihdl) &&
				...
				pr_debug("found fd=%d hdl=%pKK off=%x addr=%x\n",
						data->memory_id, ihdl,
						data->offset,
...
static void mdss_mdp_wb_free_node(struct mdss_mdp_wb_data *node)
{
	...
	if (node->user_alloc) {
		buf = &node->buf_data.p[0];
		pr_debug("free user mem_id=%d ihdl=%pKK, offset=%u addr=0x%x\n",
				...
				struct msmfb_data *data, int local)
{
	...
	if (!node) {
		...
		switch (node->state) {
		case IN_FREE_QUEUE:
			pr_err("node 0x%pKKa was already queueued before\n",
					&buf->addr);
			ret = -EINVAL;
			break;
		case IN_BUSY_QUEUE:
			pr_err("node 0x%pKKa still in busy state\n", &buf->addr);
			ret = -EBUSY;
			break;
		case WB_BUFFER_READY:
			pr_debug("node 0x%pKKa re-queueded without dequeue\n",
				&buf->addr);
		...
		case REGISTERED:
			...
			break;
		default:
			pr_err("Invalid node 0x%pKKa state %d\n",
				&buf->addr, node->state);
			ret = -EINVAL;
			break;
		}
