...
iprange_mt4(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (info->flags & IPRANGE_SRC) {
		...
		if (m) {
			pr_debug("src IP %pKI4 NOT in range %s%pKI4-%pKI4\n",
			         &iph->saddr,
	...
	if (info->flags & IPRANGE_DST) {
		...
		if (m) {
			pr_debug("dst IP %pKI4 NOT in range %s%pKI4-%pKI4\n",
			         &iph->daddr,
...
iprange_mt6(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (info->flags & IPRANGE_SRC) {
		...
		m ^= !!(info->flags & IPRANGE_SRC_INV);
		if (m) {
			pr_debug("src IP %pKI6 NOT in range %s%pKI6-%pKI6\n",
				 &iph->saddr,
				 (info->flags & IPRANGE_SRC_INV) ? "(INV) " : "",
				 &info->src_min.in6,
				 &info->src_max.in6);
	...
	if (info->flags & IPRANGE_DST) {
		...
		m ^= !!(info->flags & IPRANGE_DST_INV);
		if (m) {
			pr_debug("dst IP %pKI6 NOT in range %s%pKI6-%pKI6\n",
				 &iph->daddr,
				 (info->flags & IPRANGE_DST_INV) ? "(INV) " : "",
				 &info->dst_min.in6,
				 &info->dst_max.in6);
